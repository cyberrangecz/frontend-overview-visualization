...
import { SVG_CONFIG, SVG_MARGIN_CONFIG, AXES_CONFIG, BARS_CONFIG, CROSSHAIR_CONFIG, COLOR_SCHEME, PLAYER_POINT_CONFIG, LEVEL_LABELS_CONFIG } from './config';
import { ScaleLinear, ScaleBand, ContainerElement } from 'd3-ng2-service/src/bundle-d3';
...
export class LevelsComponent implements OnInit {
  ...
  initializeScaleBand(data: BarVisualizationData[]) {
    this.yScaleBandBars = this.d3.scaleBand()
  ...
  drawMaximumBars(barsGroup, data: BarVisualizationData[]) {
    const colorScale = this.d3.scaleOrdinal()
      .range(COLOR_SCHEME);
    barsGroup.selectAll('.score-level-bar-max')
      .data(data)
      .enter()
      .append('rect')
      .attr('id', (level: BarVisualizationData) => 'score-level-bar-max-' + level.number)
      .attr('class', 'score-level-bar score-level-bar-max')
      .attr('x', 0)
      .attr('y', (level: BarVisualizationData) => this.yScaleBandBars(level.number.toString()))
      .attr('height', this.yScaleBandBars.bandwidth())
      .attr('width', (level: BarVisualizationData) => this.xScale(level.maxTime))
  ...
  drawAverageBars(barsGroup, data: BarVisualizationData[]) {
    const colorScale = this.d3.scaleOrdinal()
      .range(COLOR_SCHEME);
    barsGroup.selectAll('.score-level-bar-avg')
      .data(data)
      .enter()
      .append('rect')
      ...
      .attr('y', (level: BarVisualizationData) => this.yScaleBandBars(level.number.toString()))
      .attr('height', this.yScaleBandBars.bandwidth())
      .attr('width', (level: BarVisualizationData) => this.xScale(level.avgTime))
      .style('fill', (d, i) => colorScale(i));
  }
 ...
 */
  drawTimeAxisLabel() {
    this.svg.append('text')
      .attr('transform', `translate(${BARS_CONFIG.width - 50}, ${AXES_CONFIG.xAxis.position.y + 26})`)
      .style("fill", "#4c4a4a")
  ...
  drawDashedLeftAxis() {
    this.svg.append('g')
      ...
      .append('path')
      .attr('d', `M0,${SVG_CONFIG.height - SVG_MARGIN_CONFIG.bottom * 0.3}.5V0.5H0`);
  }
  ...
  drawScoreAxes() {
    ...
    d3.selectAll('.score-level-bar-max').each(bar => {
      ...
      this.d3.selectAll('.y-axis>.tick>text')
  ...
  drawScoreAxesLabel() {
    this.svg.append('text')
      .attr('transform', `translate(${AXES_CONFIG.yAxis.position.x - 50}, ${SVG_CONFIG.height / 2}) rotate(-90)`)
   ...
   */
  drawPlayersOnSingleBar(players: PlayerVisualizationData[], i: number, colorScale) {
    ...
    const yBarScale = this.d3.scaleLinear()
      .range(
        [
    ...
    const xScale = this.xScale = this.d3.scaleLinear()
      .range([0, BARS_CONFIG.width])
      .domain([0, this.getMaximumTime()]);


    playersGroup.selectAll('.player-point-level-' + i)
      .data(players)
      .enter()
      .append('circle')
      .attr('class', (d: PlayerVisualizationData) => 'player-point player-point-level-' + i + ' p' + d.id)
      .attr('cx', (d: PlayerVisualizationData) => xScale(d.time))
      .attr('cy', (d: PlayerVisualizationData) => yBarScale(d.score))
      .attr('r', (d: PlayerVisualizationData) => (d.id === this.feedbackLearnerId) ? pointConfig.feedbackLearner.pointRadius : pointConfig.pointRadius)
      .style('fill', () => {
  ...
  addListeners() {
    ...
    this.d3.select('#score-level-container')
  ...
  showTooltip(player: PlayerVisualizationData) {
    ...
    const groupHeight = this.yScaleBandBars.step() * this.data.events.levels.length + SVG_MARGIN_CONFIG.top;
   ...
   * @param player data held by <circle> element in __data__ property 
   */
  onPlayerPointClick(d: PlayerVisualizationData) {
    this.d3.event.stopPropagation();
    this.outputSelectedPlayerId.emit(+d.id);
